...
Legacy browser-session option retained for compatibility:

- **Tier 1 (built-in):** `--disable-blink-features=LegacyBrowserOptionRedacted`, etc.
- **Tier 2 (manual):** ChromeDriver patching to remove `$cdc_` signatures.

This option is retained only for compatibility with historical examples.
